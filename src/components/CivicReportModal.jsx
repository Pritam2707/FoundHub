    ...
    <div className="fixed inset-0 z-50 overflow-y-auto bg-stone-900/40 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4 animate-fade-in">
      <div className="bg-white w-full max-w-xl rounded-3xl border border-stone-200 shadow-modal overflow-hidden animate-fade-in max-h-[85vh] flex flex-col">
        ...
        <div className="px-6 py-4 border-b border-stone-100 flex items-center justify-between">
          <div>
            <h2 className="text-base font-bold text-stone-900">Report an Infrastructure Issue</h2>
            <p className="text-xs text-stone-400">Potholes, lighting faults, water leaks, or road hazards</p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4.5 overflow-y-auto flex-1 text-xs">
          ...
          {/* Category */}
          <div>
            <label className="block font-semibold text-stone-700 mb-1.5">Select Category</label>
          ...
          {/* Title */}
          <div>
            <label className="block font-semibold text-stone-700 mb-1">Issue Headline *</label>
            <input
              type="text"
              required
              value={title}
              ...
              className="w-full px-3.5 py-2.5 bg-stone-50 border border-stone-200 rounded-xl text-xs focus:outline-none focus:ring-1 focus:ring-stone-900"
            />
          </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <label className="font-semibold text-stone-700">Location Geotag</label>
              <button
                type="button"
                onClick={handleAutoGPS}
                disabled={isGettingGps}
                className="text-[11px] text-stone-600 hover:text-stone-900 font-semibold flex items-center space-x-1 bg-stone-100 px-2 py-0.5 rounded-lg"
              >
          ...
          {nearbyDuplicates.length > 0 && !ignoreDuplicate && (
            <div className="p-3.5 rounded-2xl bg-amber-50/80 border border-amber-200 text-stone-900 space-y-2">
              ...
              <div className="flex items-center justify-between pt-1">
                <button
                  ...
                  className="px-3 py-1.5 bg-amber-500 text-white rounded-lg font-semibold text-xs flex items-center space-x-1"
                >
          ...
          {/* Photo */}
          <div>
            <label className="block font-semibold text-stone-700 mb-1.5">Photo Attachment</label>
            <EdgeStoreUploader
              initialUrl={imageUrl}
